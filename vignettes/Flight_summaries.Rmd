...
## Ground surface resolution 1 

*Assuming no angle of distortion on the camera*

```{r}
GSR_all_dfs=all_dfs_reduced %>% 
  rowwise() %>% 
  mutate("GSR"=GSD(H=alt_above_ground))

GSR_all_dfs %>% 
  group_by(Flight) %>% 
  summarise(mean_cm_per_pix=mean(GSR*100, na.rm=TRUE), sd_cm_per_pix=sd(GSR*100, na.rm=TRUE))

```


```{r, echo=TRUE}
resolution1=GSR_all_dfs %>% 
  mutate(Fp=Ground_footprint(GSR)) %>% 
   group_by(Flight) %>% 
  summarise(mean_GF=mean(Fp, na.rm=TRUE), sd_GF=sd(Fp, na.rm=TRUE))
resolution1
...
## Ground surface resolution 2 

*Assume angle of distortion on the camera*
...
output_summary_table=output_summary_table %>% inner_join(resolution1)
