...
```{r}
# photographs per Flight
all_dfs_reduced %>% 
  group_by(Flight) %>% 
  select(photo_no) %>% 
  summarise(Photos_per_Flight=n()) %>% 
  ggplot(aes(Flight,Photos_per_Flight, fill=Flight))+
  labs(y="Photographs per Flight")+
...
```{r}
tab=over %>% 
  select(Flight, photo_no, date, time,alt_above_ground ,grd_in_img, overlap)
