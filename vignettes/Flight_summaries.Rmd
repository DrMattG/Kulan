...
GSR_all_dfs %>% 
  group_by(Flight) %>% 
  summarise(m_per_pixel_near=mean(unlist(GSD_near), na.rm=TRUE), sd_m_per_pixel_near=sd(unlist(GSD_near), na.rm=TRUE),m_per_pixel_far=mean(unlist(GSD_far), na.rm=TRUE), sd_m_per_pixel_far=sd(unlist(GSD_far), na.rm=TRUE))
...
Export_2_excel=Export_2_excel %>% 
  dplyr::select(!c("v18","new_lat","new_lon",))# removes these coloumns as they are only used for calculating over values
